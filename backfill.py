...
def fetch_fmp_purchases(from_date, to_date):
    ...
    while page < MAX_PAGES:
        ...
        if found_old:
            print("  Stopping at page " + str(page) + " — found dates older than " + from_date)
...
def main():
    ...
    if raw:
        dates = [r.get("transactionDate","") for r in raw[:5]]
        print("  First 5 transaction dates: " + str(dates))
        last_dates = [r.get("transactionDate","") for r in raw[-5:]]
        print("  Last 5 transaction dates: " + str(last_dates))
    ...
    # Debug: show what transactionType values FMP is returning
    type_counts = {}
    for row in raw:
        tt = str(row.get("transactionType") or "").upper().strip()
        type_counts[tt] = type_counts.get(tt, 0) + 1
    print("  transactionType values in FMP response: " + str(type_counts))
    ...
    for row in raw:
        ...
        if symbol not in us_symbols:
            skipped_us += 1
            print("  Non-US skipped: " + symbol + " value=" + str(value))
            continue
        ...
        if value < MIN_VALUE:
            skipped_val += 1
            print("  Below threshold: " + symbol + " value=" + str(round(value,0)))
            continue
