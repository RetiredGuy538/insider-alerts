...
EDGAR_FEED_PAGES  = 3   # fetch this many pages of 40 = 120 filings per run
...
def fetch_edgar_form4_index():
    ...
    entries = []
    for page_num in range(EDGAR_FEED_PAGES):
        start = page_num * 40
        url = EDGAR_FEED + ("&start=" + str(start) if start > 0 else "")
        try:
            resp = requests.get(url, headers=EDGAR_HEADERS, timeout=30)
            resp.raise_for_status()
        except Exception as e:
            print("  EDGAR page " + str(page_num) + " error: " + str(e))
            break
        ...
        if len(page_entries) < 40:
            break  # last page reached
        _time.sleep(0.3)  # be polite to EDGAR between pages
    return entries
...
def parse_form4_xml(xml_text, xml_url):
    ...
    def find_text(el, *tags):
        """Find text in a tag, checking both direct text and nested <value> child."""
        for tag in tags:
            found = el.find(".//" + tag)
            if found is None:
                continue
            # Check for nested <value> element first (Form 4 XML pattern)
