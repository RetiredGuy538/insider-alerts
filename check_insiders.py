...
EDGAR_FEED        = "https://efts.sec.gov/LATEST/search-index?q=%22transactionCode%22&dateRange=custom&startdt=2026-03-01&forms=4&hits.hits._source=period_of_report,file_date,entity_name,file_num&hits.hits.total=true"
...
def fetch_form4_detail(filing_link):
    ...
    print("    === FILING INDEX (first 800 chars) ===")
    print(content[:800])
    print("    === END ===")
    ...
    try:
        ...
        print("    XML fetched OK, length=" + str(len(xml_resp.text)))
        print("    XML preview: " + xml_resp.text[:300])
...
def main():
    ...
    # ── DEBUG: only process first 2 filings to keep log manageable ──
    feed_entries = feed_entries[:2]
    print("  DEBUG: processing first 2 filings only")
    ...
    for entry in feed_entries:
        ...
        print("  Processing: " + entry["title"][:70])
        print("  Link: " + filing_link)
        ...
        print("  Transactions found: " + str(len(transactions)))
        ...
        for tx in transactions:
            ...
            print("  tx symbol=" + symbol + " value=" + str(tx.get("totalValue")) + " in_us=" + str(in_us))
