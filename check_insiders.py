...
def main():
    ...
    for entry in feed_entries:
        ...
        if transactions:
            print(f"    {entry["title"][:60]}: {len(transactions)} tx found")
            for tx in transactions[:2]:
                print(f"      sym={tx.get("symbol")} type={tx.get("transactionDate")} val={tx.get("totalValue")} us={tx.get("symbol") in us_symbols}")
        if not transactions:
            continue
