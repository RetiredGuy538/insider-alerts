...
Also writes alerts.json for the GitHub Pages dashboard.
"""
...
ALERTS_FILE       = Path("alerts.json")   # dashboard data file
...
def load_alerts() -> list:
    """Load existing dashboard alerts from disk."""
...
def save_alerts(alerts: list):
    """
    Persist alerts to alerts.json, keeping only the last DASHBOARD_DAYS days.
    Sorted newest-first so the dashboard can render without re-sorting.
    """
    cutoff = (datetime.utcnow() - timedelta(days=DASHBOARD_DAYS)).strftime("%Y-%m-%d")
    # Deduplicate by uid just in case, then prune old entries
    seen_uids = set()
    filtered = []
    for a in alerts:
...
def build_alert_record(uid: str, row: dict, value: float) -> dict:
    """Build a JSON-serializable record for the dashboard alerts.json."""
    shares = row.get("securitiesTransacted", None)
    return {
        "uid":             uid,
...
def main():
    ...
    for uid, row, value in new_alerts:
        ...
        new_records.append(build_alert_record(uid, row, value))

    if not new_alerts:
        print("  No new alerts to send.")

    # Always write alerts.json (merges new + existing, prunes old)
